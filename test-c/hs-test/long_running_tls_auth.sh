...
# Long-running TLS-Auth OpenVPN test
...
CLIENT_CONTAINER="openvpn-client-longrun"
...
# Create TAP in VPP - this creates a Linux tap interface
...
echo "=== Status ==="
echo "OpenVPN log:"
docker exec $CLIENT_CONTAINER tail -20 /tmp/openvpn.log 2>/dev/null || true
...
while true; do
    ...
    if docker exec $CLIENT_CONTAINER ping -c 1 -W 2 $VPP_TUNNEL_IP >/dev/null 2>&1; then
        ...
        [ $((fail % 5)) -eq 1 ] && {
            echo "--- debug ---"
            docker exec $CLIENT_CONTAINER tail -3 /tmp/openvpn.log 2>/dev/null || true

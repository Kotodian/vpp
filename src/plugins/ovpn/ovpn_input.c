  ...
  while (n_left_from > 0)
      ...
      if (ovpn_opcode_is_data (opcode))
	{
	  ...
	  if (opcode == OVPN_OP_DATA_V2)
	    ...
	    {
	      /* DATA_V1 - need to lookup peer by source address */
	      if (PREDICT_FALSE (len < OVPN_DATA_V1_MIN_SIZE + OVPN_TAG_SIZE))
		{
	      ...
	      /* For V1, we'd need to lookup by source IP:port */
	      /* For now, only support V2 */
...
VLIB_REGISTER_NODE (ovpn4_input_node) = {
  ...
  .next_nodes = {
    ...
    [OVPN_INPUT_NEXT_IP4_INPUT] = "ip4-input",
    [OVPN_INPUT_NEXT_IP6_INPUT] = "ip6-input",
    [OVPN_INPUT_NEXT_HANDSHAKE] = "ovpn-handshake",
			...
			vlib_frame_t *frame)
{
  ...
  while (n_left_from > 0)
	...
	if (ip_offset > 0 && ip_offset < b0->current_data)
	  {
	    u8 *start = vlib_buffer_get_current (b0) -
...
VLIB_REGISTER_NODE (ovpn_handshake_node) = {
  .function = ovpn_handshake_node_fn,
  .name = "ovpn-handshake",
  .vector_size = sizeof (u32),

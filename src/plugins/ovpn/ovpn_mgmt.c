  ...
  pool_foreach (mgmt, mm->contexts)
    {
      ...
      vlib_cli_output (vm, "  Address: %U:%u", format_ip_address, &mgmt->bind_addr,
		       mgmt->bind_port);
      vlib_cli_output (vm, "  Active clients: %u", pool_elts (mgmt->clients));
      vlib_cli_output (vm, "  Password required: %s", mgmt->password ? "yes" : "no");

		     ...
		     vlib_buffer_t *b, u32 *packet_id_out)
{
  ...
  /* AAD is the header (opcode + peer_id) */
  aad = vlib_buffer_get_current (b);
  aad_len = OVPN_OP_SIZE + OVPN_PEER_ID_SIZE;

  /* Extract packet_id */
  packet_id = clib_net_to_host_u32 (*(u32 *) (aad + aad_len));

  /* Check replay */
  if (!ovpn_crypto_check_replay (ctx, packet_id))
    return -2; /* Replay detected */

  /* Ciphertext starts after header (including packet_id in ciphertext) */
  src = aad + aad_len;
  ...
  vlib_buffer_advance (b,
		       aad_len + OVPN_PACKET_ID_SIZE); /* Skip header + pkt_id */

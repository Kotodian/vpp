...
ovpn_load_certificates (ptls_context_t *ctx, u8 *cert_data, u8 *key_data)
{
  ...
  if (key_bio)
    BIO_free (key_bio);
  /* Note: pkey is now owned by sign_cert, don't free it */
			...
			vlib_cli_command_t *cmd __attribute__ ((unused)))
{
  ...
  if (disable)
      ...
      if (omp->is_enabled && omp->options.listen_port)
	{
      ...
      clib_memset (&omp->options, 0, sizeof (omp->options));
      omp->options.sw_if_index = ~0; /* Mark as uninitialized */
      goto done;
    }
  ...
  vlib_cli_output (vm, "Using cipher algorithm: %s",
  ...
  if (!omp->is_enabled)
    {
      ...
      /* Initialize peer database for this interface */
      if (omp->options.sw_if_index != ~0)
	{
      ...
      /* Initialize pending connections database */
		      ...
		      vlib_cli_command_t *cmd __attribute__ ((unused)))
{
  ...
  vlib_cli_output (vm, "  Cipher Algorithm: %s",
		   omp->cipher_alg == OVPN_CIPHER_ALG_AES_128_GCM   ? "AES-128-GCM"
		   : omp->cipher_alg == OVPN_CIPHER_ALG_AES_256_GCM ? "AES-256-GCM"
		   : omp->cipher_alg == OVPN_CIPHER_ALG_CHACHA20_POLY1305
		     ? "CHACHA20-POLY1305"
		     : "NONE");
  if (opt->auth_name)
    vlib_cli_output (vm, "  Auth: %s", opt->auth_name);
...
VLIB_CLI_COMMAND (ovpn_enable_command, static) = {
  ...
  .short_help = "ovpn [enable|disable] [port <port>] [dev <name>] "
		...
		"[tls-crypt-key <path>] [server-addr <ip>/<len>] "
...
extern vlib_node_registration_t ovpn_output_node;
...
ovpn_init (vlib_main_t *vm)
{
  ...
  omp->ovpn_output_node_index = ovpn_output_node.index;
  ...
  /* Output uses same node for both IPv4 and IPv6 */
  omp->out4_fq_index = vlib_frame_queue_main_init (ovpn_output_node.index, 0);
  omp->out6_fq_index = omp->out4_fq_index;
		       ...
		       vlib_frame_t *f)
{
  ...
  while (1)
	...
	continue;
      ...
      /* Check each peer for rekey needs */
      ovpn_peer_t *peer;
	  ...
	  if (ovpn_peer_needs_rekey (peer, now))
	    {
	      ...
	      int rv = ovpn_peer_start_rekey (vm, peer, omp->ptls_ctx,
					      new_key_id);
	      if (rv == 0)
		{
		  ...
		  /* Note: packets will be sent by next control packet handler
		   */
		}
	    }
	}
